...
using PokemonTcgSdk.Standard.Features.CacheManager;
...
public class PokemonApiClient : IDisposable
{
    ...
    private readonly Uri _baseUri = new("https://api.pokemontcg.io/v2/");
    ...
    ///     HttpClient implementation. Should include api key in header else you will be rate limited. See
    ///     docs
    /// </param>
    ...
    {
        var url = GetApiEndpointString<T>();
    ...
    /// <param name="cancellationToken">Cancellation token for the request; not utilitized if data has been cached</param>
    /// <returns>The paged resource object</returns>
    public Task<ApiResourceList<T>> GetApiResourceAsync<T>(int take, int skip,
    ...
    /// <param name="cancellationToken">Cancellation token for the request; not utilitized if data has been cached</param>
    /// <returns>The paged resource object</returns>
    public Task<ApiResourceList<T>> GetApiResourceAsync<T>(IDictionary<string, string> filters,
        CancellationToken cancellationToken = default)
        where T : ApiResource
    {
        var url = GetApiEndpointString<T>();
    ...
    /// <param name="cancellationToken">Cancellation token for the request; not utilitized if data has been cached</param>
    /// <returns>The paged resource object</returns>
    public Task<ApiResourceList<T>> GetApiResourceAsync<T>(int take, int skip, IDictionary<string, string> filters,
        CancellationToken cancellationToken = default)
        where T : ApiResource
    {
        var url = GetApiEndpointString<T>();
        return InternalGetApiResourcePageAsync<T>(AddQueryFilterParamsToUrl(url, filters, take, skip),
            cancellationToken);
    }
    ...
    private async Task<T?> GetAsync<T>(string url, CancellationToken cancellationToken)
    {
        ...
        return await DeserializeStream<T>(await response.Content.ReadAsStreamAsync());
    }

    /// <summary>
    ///     Handles deserialization of a given stream to a given type
    /// </summary>
    private async Task<T?> DeserializeStream<T>(Stream stream)
    {
    ...
    private static string AddPaginationParamsToUrl(string uri, int? pageSize = null, int? page = null)
    {
        ...
        if (pageSize.HasValue) queryParameters.Add(nameof(pageSize), pageSize.Value.ToString());

        if (page.HasValue) queryParameters.Add(nameof(page), page.Value.ToString());

        return QueryHelpers.AddQueryString(uri, queryParameters);
    }

    private static string AddQueryFilterParamsToUrl(string uri, IDictionary<string, string> filterQuery,
